...
{
    public class ShowWorkflowGraph : IContextMenuEntry
    {
        private static NLog.ILogger _logger = NLog.LogManager.GetCurrentClassLogger();
        private const string CTX_MENUENTRY = "Show Workflow Graph";
        private const string CONST_WOINSIGNATURE = "WOIN:WOIN_";

        /// <summary>
        /// WODE:EMPL_CHANGE_CAT0; WOIN:WOIN_d477a94f4d544e28b58386a56c9cfdd4;
        /// </summary>
        ...
        public void MenuSelected(IList<int> lines, ILogLineColumnizer columnizer, ILogExpertCallback callback)
        {
            if (lines.Count == 1)
            {
                ...
                {
                    //
                    _logger.Info("do something cool with woin:WOIN_{0}", woin);
                }
                else
